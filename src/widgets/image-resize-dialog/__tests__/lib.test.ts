...
import * as loadImageModule from '~/shared/lib/load-image';
...
describe('resizeImageFromUrl', () => {
  ...
  it('должна использовать указанный формат вывода (PNG)', async () => {
    const url = 'blob:http://localhost/test';
    const percent = 50;
    const outputFormat = 'image/png';

    toBlobSpy.mockImplementation(
      (callback: (blob: Blob | null) => void, mimeType?: string) => {
    ...
    expect(toBlobSpy).toHaveBeenCalledWith(
      expect.any(Function),
      'image/png',
      undefined, // PNG не поддерживает качество
    );
  });

  it('должна использовать указанный формат вывода (WebP) с качеством', async () => {
    const url = 'blob:http://localhost/test';
    const percent = 50;
  ...
  it('должна использовать указанное качество для JPEG', async () => {
    const url = 'blob:http://localhost/test';
    const percent = 50;
    const quality = 0.7;
  ...
  it('должна использовать формат и качество для режима dimensions', async () => {
    const url = 'blob:http://localhost/test';
    const width = 800;
    const height = 600;
    const outputFormat = 'image/webp';
  ...
  it('должна игнорировать качество для PNG формата', async () => {
    const url = 'blob:http://localhost/test';
    const percent = 50;
    const outputFormat = 'image/png';
    const quality = 0.9; // Должно быть проигнорировано для PNG
    ...
    // PNG не поддерживает качество, должно быть undefined
    expect(toBlobSpy).toHaveBeenCalledWith(
      expect.any(Function),
      'image/png',
      undefined,
  ...
  it('должна использовать качество по умолчанию если не указано', async () => {
    const url = 'blob:http://localhost/test';
    const percent = 50;
    ...
    // Должно использоваться качество по умолчанию (1.0)
    expect(toBlobSpy).toHaveBeenCalledWith(
      expect.any(Function),
      'image/jpeg',
      1.0,
...
describe('prepareResizeParams', () => {
  ...
  it('должна включать outputFormat и quality в параметры для режима процентов', () => {
    const imageUrl = 'blob:http://localhost/test';
    const mode = 'percent';
    const percent = 150;
    const width = 1920;
    ...
    const result = prepareResizeParams(
      imageUrl,
      mode,
      percent,
      width,
  ...
  it('должна включать outputFormat и quality в параметры для режима размеров', () => {
    const imageUrl = 'blob:http://localhost/test';
    const mode = 'dimensions';
    const percent = 100;
    const width = 1920;
  ...
  it('должна работать без outputFormat и quality (обратная совместимость)', () => {
    const imageUrl = 'blob:http://localhost/test';
    const mode = 'percent';
    const percent = 150;
    const width = 1920;
    ...
    // Проверяем, что свойства есть, но undefined (опциональные параметры)
    expect('outputFormat' in result).toBe(true);
    expect('quality' in result).toBe(true);
    expect(result.outputFormat).toBeUndefined();
    expect(result.quality).toBeUndefined();
  });
});

describe('downloadResizedImage - формирование имени файла', () => {
  let mockFileDownloadDirect: ReturnType<typeof vi.fn>;
  ...
  beforeEach(() => {
    ...
    mockFileDownloadDirect = vi.mocked(fileDownloadDirect);
    ...
    // Мокируем loadImageFromUrl
    vi.spyOn(loadImageModule, 'loadImageFromUrl').mockResolvedValue(mockImage);
  ...
  it('должен формировать имя файла с расширением из формата вывода', async () => {
    ...
    await libModule.downloadResizedImage({
      params: {
        mode: 'percent',
        imageUrl: 'blob:test',
        percent: 100,
    ...
    expect(mockFileDownloadDirect).toHaveBeenCalledTimes(1);
    const [, filename] = mockFileDownloadDirect.mock.calls[0];

    // Используется расширение из blob (png)
    expect(filename).toBe('photo_resized.png');
  });

  it('должен формировать имя файла для файла без расширения', async () => {
    ...
    expect(filename).toBe('image_resized.jpg');
  });

  it('должен использовать resized-image.extension если originalFile null', async () => {
    const mockBlob = new Blob(['test'], { type: 'image/jpeg' });
    ...
    expect(filename).toBe('resized-image.jpg');
  });

  it('должен использовать расширение из формата вывода', async () => {
    const mockFile = new File(['test'], 'photo.jpg', { type: 'image/jpeg' });
    const mockBlob = new Blob(['test'], { type: 'image/webp' }); // Формат вывода WebP
    ...
    // Расширение должно быть из формата вывода (.webp)
    expect(filename).toBe('photo_resized.webp');
  });
});
